...
class GeneralManagerService {
  static async saveGeneralManager(generalManager) {
  ...
  // This function should get all the generalManagers from the generalManagers dexie store, along with 
  // the team associated with the general manager, if applicable.  The team record has a field on it 
  // that indicates the General Manager ID. 
  static async getGeneralManagers(leagueId) {
    let gms = await db.generalManagers.where("leagueId").equals(leagueId).toArray();
    for (let i = 0; i < gms.length; i++) {
      const gm = gms[i];

      // get the team where the generalManagerId = gm.generalManagerId
      gm.team = await db.teams.where("gmId").equals(gm.generalManagerId).first();
    }
    return gms;
  }
